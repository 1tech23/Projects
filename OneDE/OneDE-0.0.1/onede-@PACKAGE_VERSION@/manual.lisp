  ...
  (ppcre:register-groups-bind (name) ("^@@@ (.*)" line)
                              ...
                              (let ((fn (if (find #\( name :test 'char=)
                                            ...
                                            (with-standard-io-syntax
                                              (let ((*package* (find-package :onede)))
                                                (fdefinition (read-from-string name))))
                                            (symbol-function (find-symbol (string-upcase name) :onede))))
                                    (*print-pretty* nil))
...
(defun generate-macro-doc (s line)
  (ppcre:register-groups-bind (name) ("^%%% (.*)" line)
                              (dprint name)
                              (let* ((symbol (find-symbol (string-upcase name) :onede))
                                     (*print-pretty* nil))
...
(defun generate-variable-doc (s line)
  (ppcre:register-groups-bind (name) ("^### (.*)" line)
                              (dprint name)
                              (let ((sym (find-symbol (string-upcase name) :onede)))
...
(defun generate-command-doc (s line)
  (ppcre:register-groups-bind (name) ("^!!! (.*)" line)
                              (dprint name)
                              (let ((cmd (symbol-function (find-symbol (string-upcase name) :onede))))
...
(defun generate-manual (&key (in #p"onede.texi.in") (out #p"onede.texi"))
