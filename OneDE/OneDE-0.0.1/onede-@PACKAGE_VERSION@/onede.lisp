...
(defun load-rc-file (&optional (catch-errors t))
  "Load the user's .onederc file or the system wide one if that
  ...
  (let* ((xdg-config-dir
           (let ((dir (getenv "XDG_CONFIG_HOME")))
             (if (or (not dir) (string= dir ""))
                 ...
                 dir)))
         (user-rc
           (probe-file (merge-pathnames #p".onederc" (user-homedir-pathname))))
         (dir-rc
           (probe-file (merge-pathnames #p".onede.d/init.lisp" (user-homedir-pathname))))
         (conf-rc
           (probe-file (merge-pathnames #p"onede/config" xdg-config-dir)))
         (etc-rc (probe-file #p"/etc/onederc"))
...
(defun error-handler (display error-key &rest key-vals &key asynchronous &allow-other-keys)
  ...
  (cond 
    ...
    ((eq error-key 'xlib:access-error)
     (write-line "Another window manager is currently running. It would be wise to shut one of them down.")
     (throw :top-level :quit))
     ;; all other asynchronous errors are printed.
     (asynchronous
      (message "Caught an asynchronous error with the X server: ~s ~s" error-key key-vals))
     (t
...
(defvar *in-main-thread* nil
  "Dynamically bound to T during the execution of the main onede function.")
...
(defmethod handle-top-level-condition ((c serious-condition))
  (when (and (find-restart :remove-channel)
             (not (typep *current-io-channel*
                         '(or onede-timer-channel display-channel request-channel))))
    (message "Removed channel ~S due to some very *unfortunate* unknown error '~A'." *current-io-channel* c)
...
(defclass onede-timer-channel () ())

(defmethod io-channel-ioport (io-loop (channel onede-timer-channel))
  (declare (ignore io-loop))
  nil)
(defmethod io-channel-events ((channel onede-timer-channel))
  (with-lock-held (*timer-list-lock*)
    (if *timer-list*
        `((:timeout ,(timer-time (car *timer-list*))))
        '(:loop))))
(defmethod io-channel-handle ((channel onede-timer-channel) (event (eql :timeout)) &key)
  (run-expired-timers))
(defmethod io-channel-handle ((channel onede-timer-channel) (event (eql :loop)) &key)
  (run-expired-timers))
...
(defun onede-internal (display-str)
  (multiple-value-bind (host display screen protocol) (parse-display-string display-str)
    ...
    (with-simple-restart (quit-onede "Quit OneDE")
           ...
           (progn
             (let ((*initializing* t))
               ...
               (when *last-unhandled-error*
                 (message-no-timeout "^B^1*OneDE Crashed With An Unexplicable Error... Well, crud.~%In order to copy the error to the clipboard, run the 'copy-unhandled-error' command.~%^b~a^B^n~%~%~a"
...
;; Usage: (onede)
(defun onede (&optional (display-str (or (getenv "DISPLAY") ":0")))
  "Start the One Desktop Environment."
