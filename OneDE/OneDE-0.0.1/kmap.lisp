...
\(defvar *my-frame-bindings*
  (let ((m (onede:make-sparse-keymap)))
    (onede:define-key m (onede:kbd \"f\") \"curframe\")
    (onede:define-key m (onede:kbd \"M-b\") \"move-focus left\")
    m ; NOTE: this is important
  ))

\(onede:define-key onede:*root-map* (onede:kbd \"C-f\") '*my-frame-bindings*)
@end example"
  (make-kmap))
...
(define-condition kbd-parse-error (onede-error)
...
kbd-parse if the key failed to parse."
  (let* ((p (when (> (length string) 2)
         ...
         (keysym (onede-name->keysym (subseq string (if p (1+ p) 0)))))
    (if keysym
...
(defun print-key (key)
  (format nil "~a~a"
          (print-mods key)
          (keysym->onede-name (key-keysym key))))
...
@example
\(onede:define-key onede:*root-map* (onede:kbd \"C-z\") \"echo Zzzzz...\")
@end example
