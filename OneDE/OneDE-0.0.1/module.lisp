...
;; Commentary:
;;
;; Use `set-module-dir' to set the location onede searches for modules.
...
(defvar *module-dir*
  (pathname-as-directory (concat (getenv "HOME") "/.onede.d/modules"))
...
(defvar *load-path* nil
  ...
  configure script when OneDE was built, or later by the user using
  `add-to-load-path'")
...
called each time OneDE starts with the argument `*module-dir'"
...
(defun set-module-dir (dir)
  "Sets the location of the for OneDE to find modules"
  (when (stringp dir)
    (setf dir (pathname (concat dir "/"))))
  (setf *module-dir* dir)
  (init-load-path *module-dir*))

(define-onede-type :module (input prompt)
