...
(defun completing-read (screen prompt completions &key (initial-input "") require-match)
  "Read a line of input through onede and return it with TAB
...
(defun read-one-line (screen prompt &key (initial-input "") require-match password)
  "Read a line of input through onede and return it. returns nil if the user aborted."
...
;; (defun x11mod->onemod (screen state)
;;   (let ((mod nil))
...
(defun mod->string (state)
  "Convert a onede modifier list to a string"
...
;;   (values (xlib:keycode->keysym *display* code 0) (x11mod->onemod state)))
