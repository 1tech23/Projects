;; Copyright (C) 2022 Logan Alldregde
;;
...
(defmacro define-onede-event-handler (event keys &body body)
  (let ((fn-name (gensym))
        (event-slots (gensym)))
...
(flet ((has-x (mask) (= 1 (logand mask 1)))
  ...
  (defun configure-unmanaged-window (xwin x y width height border-width value-mask)
    "Call this function for windows that onede isn't
...
(define-onede-event-handler :configure-request (stack-mode #|parent|# window #|above-sibling|# x y width height border-width value-mask)
...
(define-onede-event-handler :configure-notify (stack-mode #|parent|# window #|above-sibling|# x y width height border-width value-mask)
...
(define-onede-event-handler :map-request (parent send-event-p window)
  (unless send-event-p
...
(define-onede-event-handler :unmap-notify (send-event-p event-window window #|configure-p|#)
...
(define-onede-event-handler :destroy-notify (send-event-p event-window window)
...
(define-onede-event-handler :key-press (code state #|window|#)
...
(defun handle-onede-commands (root)
  "Handle a OneDE style command request."
  (let* ((win root)
         (screen (find-screen root))
         (data (xlib:get-property win :onede_command :delete-p t))
         (cmd (bytes-to-string data)))
    (let ((msgs (screen-last-msg screen))
          (hlts (screen-last-msg-highlights screen))
          (*executing-onede-command* t))
      (setf (screen-last-msg screen) '()
            (screen-last-msg-highlights screen) '())
      (eval-command cmd)
      (xlib:change-property win :onede_command_result
...
(define-onede-event-handler :property-notify (window atom state)
  ...
  (case atom
    ...
    (:onede_command
     ...
     (let* ((screen (find-screen window)))
       (when (and (eq state :new-value)
                  screen)
         (handle-onede-commands window))))
    (t
     (let ((window (find-window window)))
       (when window
         (update-window-properties window atom))))))

(define-onede-event-handler :mapping-notify (request start count)
...
(define-onede-event-handler :selection-request (requestor property selection target time)
  (send-selection requestor property selection target time))

(define-onedeump-event-handler :selection-clear (selection)
...
(define-onede-event-handler :exposure (window x y width height count)
  (let (screen ml)
    (when (zerop count)
      (cond
...
(define-onede-event-handler :reparent-notify (window parent)
  (let ((win (find-window window)))
    (when (and win
...
(define-onede-event-handler :client-message (window type #|format|# data)
...
(define-onede-event-handler :focus-out (window mode kind)
  (dformat 5 "~@{~s ~}~%" window mode kind))

(define-onede-event-handler :focus-in (window mode kind)
  (let ((win (find-window window)))
...
(define-onede-event-handler :enter-notify (window mode)
...
(define-onede-event-handler :button-press (window code x y child time)
  (let (screen ml win)
    (cond
...
(defun handle-event (&rest event-slots &key display event-key &allow-other-keys)
  ...
  (let ((eventfn (gethash event-key *event-fn-table*))
    ...
    (when eventfn
      ...
      ;; in onede. So far the only slot that seems to be affected is
      ...
      (handler-case
          (progn
            ;; This is not the onede top level, but if the restart
            ...
            (with-simple-restart (top-level "Return to onede's top level")
