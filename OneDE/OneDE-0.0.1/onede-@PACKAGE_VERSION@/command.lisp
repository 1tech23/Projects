...
(defvar *command-hash* (make-hash-table :test 'eq)
  "A list of interactive onede commands.")
...
@item :module
An existing onede module
@end table

Note that new argument types can be created with DEFINE-ONEDE-TYPE.

PROMPT can be string. In this case, if the corresponding argument is
missing from an interactive call, onede will use prompt for its
...
(defmacro define-onede-command (name (&rest args) &body body)
...
(defmacro define-onede-type (type (input prompt) &body body)
...
@example
\(define-onede-type :symbol (input prompt)
		     ...
		     (or (argument-pop input)
		         ...
		         (string-trim \" \"
		           (completing-read (current-screen)
					  ...
					  ;;  onede package.
					  (let (acc)
					    (do-symbols (s (find-package \"ONEDE\"))
					      (push (string-downcase (symbol-name s)) acc))
					    acc)))
                      (throw 'error \"Abort.\")))
                  \"ONEDE\")
     (throw 'error \"Symbol not in ONEDE package\")))
...
symbol in the onede package. The command @code{symbol} uses it and
then describes the symbol."
  `(setf (gethash ,type *command-type-hash*)
    (lambda (,input ,prompt)
      ,@body)))

(define-onede-type :y-or-n (input prompt)
...
(define-onede-type :variable (input prompt)
  (lookup-symbol (argument-pop-or-read input prompt)))

(define-onede-type :function (input prompt)
...
(define-onede-type :command (input prompt)
  (or (argument-pop input)
      (completing-read (current-screen)
                       prompt
                       (all-commands))))

(define-onede-type :key-seq (input prompt)
  (labels ((update (seq)
             (message "~a: ~{~a ~}"
                      prompt
...
(define-onede-type :window-number (input prompt)
...
(define-onede-type :number (input prompt)
  (let ((n (or (argument-pop input)
               (read-one-line (current-screen) prompt))))
    (when n
      (handler-case
...
(define-onede-type :string (input prompt)
  (or (argument-pop input)
      (read-one-line (current-screen) prompt)))

(define-onede-type :password (input prompt)
  (or (argument-pop input)
      (read-one-line (current-screen) prompt :password t)))

(define-onede-type :key (input prompt)
  (let ((s (or (argument-pop input)
               (read-one-line (current-screen) prompt))))
    (when s
      (kbd s))))

(define-onede-type :window-name (input prompt)
  (or (argument-pop input)
      (completing-read (current-screen) prompt
                       (mapcar 'window-name
                               (group-windows (current-group))))))

(define-onede-type :direction (input prompt)
  (let* ((values '(("up" :up)
                   ("down" :down)
                   ("left" :left)
                   ("right" :right)))
...
(define-onede-type :gravity (input prompt)
...
(define-onede-type :group (input prompt)
...
(define-onede-type :frame (input prompt)
...
(define-onede-type :shell (input prompt)
  (or (argument-pop-rest input)
      (completing-read (current-screen) prompt 'complete-program)))

(define-onede-type :rest (input prompt)
  (or (argument-pop-rest input)
      (read-one-line (current-screen) prompt)))
...
(defun run-commands (&rest commands)
  "Run each onede command in sequence. This could be used if you're
