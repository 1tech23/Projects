...
(defvar *start-hook* '()
  "A hook called when onede starts.")

(defvar *quit-hook* '()
  "A hook called when onede quits.")

(defvar *restart-hook* '()
  "A hook called when onede restarts.")

(defvar *internal-loop-hook* '()
  "A hook called inside onede's inner loop.")

(defvar *event-processing-hook* '()
  "A hook called inside onede's inner loop, before the default event
...
(defvar *message-hook* '()
  "A hook called whenever onede displays a message. The hook
...
(defvar *focus-group-hook* '()
  "A hook called whenever onede switches groups. It is called with 2 arguments: the current group and the last group.")
...
;; Data types and globals used by onede
...
ONEDE-INTERNAL-LOOP, then this variable decides what action
...
to login remotely to regain control. :abort quits onede.")
...
(defclass screen ()
  ((id :initarg :id :reader screen-id)
   ...
   (withdrawn-windows :initform () :accessor screen-withdrawn-windows :documentation
    "A list of withdrawn windows. These are of type onede::window
...
(defvar *screen-list* '()
  "The list of screens managed by onede.")

(defvar *initializing* nil
  "True when starting onede. Use this variable in your rc file to
run code that should only be executed once, when onede starts up and
loads the rc file.")

(defvar *processing-existing-windows* nil
  "True when processing pre-existing windows at startup.")

(defvar *executing-onede-command* nil
...
\(defun my-rad-fn (to-frame from-frame)
  (onede:message \"Mustard!\"))

\(onede:add-hook onede:*focus-frame-hook* 'my-rad-fn)
@end example"
...
(defvar *debug-expose-events* nil
  "Set this variable for a visual indication of expose events on internal OneDE windows.")
...
if you want everything to go to ~/.onede.d/debug-output.txt you would
do:
...
(defvar *x-selection* nil
  "This is a plist of onede's current selections. The different properties are
...
(defvar *deny-map-request* nil
  "A list of window properties that onede should deny matching windows'
...
(defvar *startup-message* "^2*Welcome to The ^BOne^b ^BW^besktop ^BM^benvironment!
Press ^5*~a ?^2* for help."
  "This is the message OneDE displays when it starts. Set it to NIL to
suppress.")

(defvar *default-package* (find-package '#:onede-user)
  "This is the package eval reads and executes in. You might want to set
this to @code{:onede} if you find yourself using a lot of internal
onede symbols. Setting this variable anywhere but in your rc file
will have no effect.")
...
@var{lock} and @var{raise} are both non-nil, then onede will jump to
...
onede ignores the mouse. :sloppy means input focus follows the
...
(defvar *data-dir* nil
  "The directory used by onede to store data between sessions.")

(defun data-dir-file (name &optional type)
  "Return a pathname inside onede's data dir with the specified name and type"
...
(defmacro with-data-file ((s file &rest keys &key (if-exists :supersede) &allow-other-keys) &body body)
  "Open a file in OneDE's data directory. keyword arguments are sent
...
(define-condition onede-condition (condition)
  ((message :initarg :message :reader warning-message))
  (:documentation "Any onede specific condition should inherit from this.")
  (:report (lambda (condition stream)
            (format stream "~A~%" (warning-message condition)))))

(define-condition onede-error (onede-condition error)
  ()
  (:documentation "Any onede specific error should inherit this."))

(define-condition onede-warning (warning onede-condition)
  ()
  (:documentation "Adds a message slot to warning. Any onede specific warning
